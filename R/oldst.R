...
#'
# code copy/pasted from mean_age.R; only difference is rather than calling
# mean(ages), search for max value in ages vector
...
oldst <- function(x) {
  if (x == 2013) {
    ...
    paste("Henry J. Bruton, age", max(ages))
    ...
    ages <- x - years_numvec + 22
    paste("Charles B. Dew and Donald deB. Beaver, age", max(ages))
  } else {
    print("argument of yngst function must be an integer representing a year from 2004 to 2014")
  }
}
